...
const Lobby = ({ user, onJoin, onStartLocal }) => {
    
    const createGame = async () => {
        const newId = Math.random().toString(36).substring(2, 8).toUpperCase();
        const initialState = {
            host: { uid: user.uid, name: user.displayName || 'Host' },
            guest: null,
            status: 'waiting',
            lines: [],
            triangles: [],
    ...
    return (
        <div className="flex flex-col items-center justify-center h-screen bg-slate-900 text-white gap-6">
            ...
            <p className="text-slate-500 text-xs mt-4">v1.0.0 Alpha</p>
        </div>
    );
};
