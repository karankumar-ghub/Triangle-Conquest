...
const GameBoard = ({ gameId, user }) => {
    const [isMuted, setIsMuted] = useState(localStorage.getItem('triangle_mute') === 'true');
    const toggleMute = () => {
        const newState = !isMuted;
        setIsMuted(newState);
        localStorage.setItem('triangle_mute', newState);
    };

    const canvasRef = useRef(null);
    const { gameState, diceValue, isRolling, handleRoll, interaction, myRole, toast, timeLeft, handleExit, rollTrigger } = useGameLogic(canvasRef, gameId, user);
    ...
    // RESOLVE NAMES
    const p1Name = gameId ? (gameState.host?.name || 'PLAYER 1') : (user.displayName || 'PLAYER 1');
    const p2Name = gameId ? (gameState.guest?.name || 'PLAYER 2') : 'PLAYER 2';
    ...
    return (
        <div className="flex flex-col h-screen w-full bg-[#0f172a] text-white font-sans overflow-hidden select-none relative">
            ...
            <div className="flex justify-between items-center p-4 h-16 bg-slate-900/50 backdrop-blur-md z-10 shrink-0">
                <button onClick={handleExit} className="text-red-400 border border-red-500/30 px-4 py-1 rounded-full text-xs font-bold hover:bg-red-500/10 transition-colors">EXIT</button>
                <div className="text-center relative">
                    ...
                    <h1 className={`text-xl font-black tracking-widest ${gameState.turn === 1 ? "text-blue-400" : "text-green-400"} animate-pulse`}>
                        {gameState.turn === 1 ? (p1Name.split(' ')[0]) : (p2Name.split(' ')[0])}
                    </h1>
                ...
                <button onClick={toggleMute} className="w-8 h-8 rounded-full bg-slate-800 border border-slate-600 flex items-center justify-center">
                    {isMuted ? '🔇' : '🔊'}
                </button>
            </div>
            ...
            <div className="h-32 bg-slate-900/80 p-4 flex items-center justify-between gap-4 max-w-2xl mx-auto w-full rounded-t-3xl border-t border-white/10 backdrop-blur-lg mb-0 shrink-0 safe-area-bottom">
                ...
                <div className={`flex-1 h-full rounded-2xl flex flex-col items-center justify-center transition-all duration-300 relative ${gameState.turn === 1 ? 'bg-blue-600/20 border-2 border-blue-500' : 'bg-white/5 border border-white/5 grayscale opacity-50'}`}>
                    <span className="text-2xl mb-1">{gameId ? (gameState.host?.photo || '🤖') : (user.photoURL || '🤖')}</span>
                    <span className="text-[10px] font-bold text-blue-300 max-w-[80px] truncate">{p1Name}</span>
                    <span className="text-3xl font-black text-white">{gameState.scores[1]}</span>
                    <div className="absolute top-2 right-2 flex gap-1">
                        {[...Array(gameState.warnings?.[1] || 0)].map((_, i) => (<div key={i} className="w-2 h-2 rounded-full bg-red-500 animate-pulse"></div>))}
                    </div>
                </div>

                {/* 3D Dice */}
                <div className="flex flex-col items-center justify-center w-24 relative z-20">
                    <div className={`perspective-container cursor-pointer transition-transform ${gameState.moves > 0 ? 'opacity-50 cursor-not-allowed' : 'active:scale-95'}`} onClick={handleRoll}>
                    ...
                    <div className="absolute -bottom-6 text-[9px] text-slate-600 font-mono hidden md:block">[SPACE]</div>
                </div>
                ...
                <div className={`flex-1 h-full rounded-2xl flex flex-col items-center justify-center transition-all duration-300 relative ${gameState.turn === 2 ? 'bg-green-600/20 border-2 border-green-500' : 'bg-white/5 border border-white/5 grayscale opacity-50'}`}>
                    <span className="text-2xl mb-1">{gameId ? (gameState.guest?.photo || 'Waiting...') : '👽'}</span>
                    <span className="text-[10px] font-bold text-green-300 max-w-[80px] truncate">{p2Name}</span>
                    <span className="text-3xl font-black text-white">{gameState.scores[2]}</span>
                    <div className="absolute top-2 right-2 flex gap-1">
                        {[...Array(gameState.warnings?.[2] || 0)].map((_, i) => (<div key={i} className="w-2 h-2 rounded-full bg-red-500 animate-pulse"></div>))}
                    </div>
                </div>
            </div>
