...
export const useGameLogic = (canvasRef, gameId, user) => {
    ...
    const handleRoll = async () => {
        ...
        // 2. UPDATE VISUALS IMMEDIATELY (Starts animation)
        setDiceValue(roll);

        // 3. UPDATE GAME LOGIC AFTER ANIMATION (1 second delay)
        setTimeout(async () => {
            setIsRolling(false); // Unlock input
        ...
        }, 1000); // 1000ms matches the CSS transition time + buffer
    };
    ...
    const handleMove = async (p1, p2) => {
        ...
        if (nextMoves === 0) nextTurn = gameState.turn === 1 ? 2 : 1;
        ...
        const updates = { lines: nextLines, triangles: nextTrianglesList, moves: nextMoves, scores: nextScores, turn: nextTurn, status: status };
    ...
    const interaction = {
        onDown: (e) => {
            ...
            const x = ((cx - r.left) * (canvasRef.current.width / r.width)) / dpr;
            const y = ((cy - r.top) * (canvasRef.current.height / r.height)) / dpr;
